    ...
    >
      ...
      <div className="mx-auto max-w-7xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
        {WhyUsData.map((item) => (
          <div
            key={item.id}
            className="bg-[#1B3EC4] rounded-xl p-4 sm:p-5 md:p-6 flex flex-col items-center text-center h-full transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl min-h-[180px]"
          >
            <div className="text-white hover:cursor-default space-y-2">
              {item.textKeys.map((key, index) => (
                <div
                  key={index}
                  className="font-extrabold text-2xl sm:text-3xl md:text-4xl leading-snug"
                >
                  {t(key)}
                </div>
              ))}
